...
Nautilus middleware is the behavior-changing companion to observer hooks.
...
- Rewrite LLM provider request kwargs before Nautilus calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Nautilus retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Nautilus guardrails,
...
Nautilus stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Nautilus runs
them as a nested chain in registration order. Middleware failures are fail-open:
Nautilus logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Nautilus applies middleware in this order:
...
For each tool call, Nautilus applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Nautilus pre-execution path against the effective arguments:
...
```bash
nautilus plugins enable <plugin-name>
```
...
mkdir -p "$HERMES_HOME"
nautilus plugins enable <plugin-name>
nautilus chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run nautilus plugins enable <plugin-name>
uv run nautilus chat --query 'Reply exactly ok'
```
...
- If execution middleware raises before calling `next_call(...)`, Nautilus treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Nautilus preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Nautilus does not convert downstream
