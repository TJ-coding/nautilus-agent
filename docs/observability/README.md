# Nautilus Observer Hooks

Nautilus observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Nautilus' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Nautilus catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Nautilus only builds
