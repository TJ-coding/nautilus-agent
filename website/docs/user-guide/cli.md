...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Nautilus Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Nautilus Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `nautilus setup --portal` — and you're ready to `nautilus chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Nautilus also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `nautilus --tui` — see the [TUI](tui.md) guide.
:::
...
nautilus chat --model "anthropic/claude-sonnet-4"

# With a specific provider
nautilus chat --provider nous        # Use Nous Portal
nautilus chat --provider openrouter  # Force OpenRouter

# With specific toolsets
nautilus chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
nautilus -s hermes-agent-dev,github-auth
nautilus chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
nautilus --continue             # Resume the most recent CLI session (-c)
nautilus --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
nautilus chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
nautilus -w                         # Interactive mode in worktree
nautilus -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Nautilus CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Nautilus CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `nautilus --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`nautilus -c` or `nautilus --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Nautilus to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Nautilus supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Nautilus replies |
...
```bash
nautilus -s hermes-agent-dev,github-auth
nautilus chat -s github-pr-workflow -s github-auth
```

Nautilus loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Nautilus recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Nautilus — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Nautilus is working, Nautilus prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Nautilus to the background — just like any terminal process. The shell prints a confirmation:

```
Nautilus Agent has been suspended. Run `fg` to bring Nautilus Agent back.
```
...
Resume this session with:
  nautilus --resume 20260225_143052_a1b2c3
...
```bash
nautilus --continue                          # Resume the most recent CLI session
nautilus -c                                  # Short form
nautilus -c "my project"                     # Resume a named session (latest in lineage)
nautilus --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
nautilus --resume "refactoring auth"         # Resume by title
nautilus -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `nautilus sessions rename <id> <title>` from the command line. Use `nautilus sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Nautilus's SQLite state database under `~/.hermes/state.db`. The database keeps:
...
Nautilus immediately confirms the task and gives you back the prompt:
